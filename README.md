...
**Terraform** provisions and coordinates the infrastructure lifecycle — Resource Group, networking, registry, Kubernetes cluster, PostgreSQL, Key Vault — and triggers the platform bootstrap when infrastructure, migrations, or bootstrap logic change.
...
7. **Add controlled developer actions** — restart, image change, scale, resize, enable a capability — all updating desired state through GitOps rather than bypassing it.
